        ...
        if let _ = delegate, let contentController = contentController {
            ...
            if !validated && allowed {
                ...
                if let mode = calendarMode {
                    ...
                    let frameToUpdate = (selfSize != contentViewSize ? bounds : CGRectZero)
                                        
                    contentController.updateFrames(frameToUpdate)
                }
            }
        }
    }
...
extension CVCalendarView {
    ...
    public func changeMode(mode: CalendarMode, completion: () -> () = {}) {
        if let selectedDate = coordinator.selectedDayView?.date.convertedDate() where calendarMode != mode {
            ...
            case .WeekView:
                if dayViewSize == nil {
                    contentController.updateHeight(0, animated: true)
                }
                else {
            ...
            case .MonthView:
                ...
                print("MonthView, updateHeight:\(contentController.presentedMonthView.potentialSize.height), bounds:\(bounds)")
            }
