...
public final class CVCalendarMonthContentViewController: CVCalendarContentViewController {
    ...
    public init(calendarView: CalendarView, frame: CGRect, presentedDate: NSDate) {
        monthViews = [Identifier : MonthView]()
        super.init(calendarView: calendarView, frame: frame)
//        print("frame:\(frame), scrollView.bounds:\(scrollView.bounds) at CVCalendarMonthContentViewController init")
...
//        print("monthViews[Presented]:\(monthViews[Presented]), calendarView.coordinator.selectedDayView?.date:\(calendarView.coordinator.selectedDayView?.date)")
        if let presented = monthViews[Presented] {
            var selectedDate = calendarView.coordinator.selectedDayView?.date
            if selectedDate == nil {
                selectedDate = Date(date: dateAfterDate(date))
            }
//            print("presentedDate:\(presentedDate.commonDescription), selectedDate:\(selectedDate?.commonDescription)")
            if !matchedDays(selectedDate!, presentedDate) && !togglingBlocked {
                if !matchedMonths(presentedDate, selectedDate!) {
                    ...
                    UIView.animateWithDuration(0.0, delay: 0, options: UIViewAnimationOptions.CurveEaseInOut, animations: {
                        presented.alpha = 0
...
extension CVCalendarMonthContentViewController {
    ...
    public func updateSelection() {
        ...
        if let presentedMonthView = monthViews[Presented] {
            ...
            calendarView.presentedDate = Date(date: calendarView.manager.monthDateRange(presentedMonthView.date).monthEndDate)
            ...
            if let selected = coordinator.selectedDayView, let selectedMonthView = selected.monthView where !matchedMonths(Date(date: selectedMonthView.date), Date(date: presentedMonthView.date)) && calendarView.shouldAutoSelectDayOnMonthChange {
                ...
                if matchedMonths(current, presented) {
                    ...
                    selectDayViewWithDay(Date(date: calendarView.manager.monthDateRange(presentedMonthView.date).monthEndDate).day, inMonthView: presentedMonthView)
                }
            }
        }
